...
var govNav = {
	...
	setupmenu:function($menu, setting){
		...
		$topul.find('li>div, li>ul').each(function(){ // find drop down elements
			...
			$parentli
				...
				//	}
					...
					// RA-904: Remove active classes from theme menu items
					$mainlis.removeClass('default');			
					$mainlis.find('span.level-trigger').removeClass('open');			
			...
			$parentli.find(".level-trigger")
				.on('click', function(e){	
					if($(this).hasClass("open")){
						...
						$(".firstLevel > li").each(function(index) {
		...
		var $mainlis = $topul.children('li.hassub').on('mouseleave', function(){
			// RA-904: Do not collapse menu item when mouse leaves 
			//govNav.showhide($(this), 'hide', setting)
			
			// RA-904: Remove active classes from theme menu items
			$mainlis.removeClass('selected open');			
			$mainlis.find('span.level-trigger').removeClass('open');
		})
	},
